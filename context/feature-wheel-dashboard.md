...
- [ ] **This session's ACTIVE box redesign + MACD Trend + weekly-exhaustion changes are uncommitted** on `develop` (8 files modified: `cc_timing_signal.py`, `sp_timing_signal.py`, `technicals_fetcher.py`, their 3 test files, `WheelDashboardPage.tsx`, `types/index.ts`). Unlike the CC/SP Timing Signal features, this round was done via direct edits on `develop` (no worktree, no plan file) since each change was small/localized — verified after every step with `tsc --noEmit` + `eslint` + `pytest`, but never committed. Needs a commit (or several) before this is durable.
- [ ] **The `context/feature-wheel-dashboard.md` update itself was previously committed by the user directly** (commit `08a769c`, "initial change for the Timing of CSP and CC added") — this session's context update (the one you're reading) is layered on top of that and is itself uncommitted along with the code changes above.
- [ ] **New MACD Trend / exhaustion features not yet visually verified in a live browser** — same caveat as the CC/SP Timing badges before them. The exhaustion adjustment WAS verified live via curl against `/api/market/cc-timing-signal/AAPL`, but the "MACD Trend" 3-pill UI column has only been verified via `tsc`/`eslint`, not clicked through in the browser.
- [ ] **Worktree `.claude/worktrees/sp-timing-signal` cleanup still pending** (unchanged from before) — still on disk, still fully merged/redundant with `develop` at `987d5e3`. Note `develop` has since moved past that commit (context-file commit `08a769c`, plus this session's uncommitted work), so it's no longer literally identical to `develop`'s tip, but the worktree branch's own commits are still fully contained in `develop`'s history — safe to remove the same way (`ExitWorktree`, `action: "remove"`, `discard_changes: true`).
