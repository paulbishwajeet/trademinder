...
**Status:** Active development — CC/SP Timing Signals, ACTIVE box redesign, MACD Trend column, and RSI(D) Trend crossover redesign all committed and pushed as of 2026-09-17; not yet manually verified in a live browser
...
`develop` branch, up to date with `origin/develop` — this session's RSI(D) Trend redesign (4 files: `backend/app/services/cc_timing_signal.py`, `backend/app/services/sp_timing_signal.py`, `backend/tests/test_cc_timing_signal.py`, `backend/tests/test_sp_timing_signal.py`) plus this context-file update have been committed and pushed. Nothing from this feature is uncommitted as of the end of this session. Backend: 302 passed, same 6 pre-existing unrelated failures as always (test_cc_signal.py x4, test_market_router_p2.py::test_prefetch_still_501, test_schwab_client.py::test_schwab_alias_maps_spx). Frontend: `tsc --noEmit` clean, `eslint src/pages/WheelDashboardPage.tsx` at the pre-existing 3-problem baseline.

**The single next action:** everything code-wise is committed and pushed, so the next real task is the manual-browser-verification item that's been carried forward for a few sessions now (see Open Questions) — nobody has actually clicked through `/wheel` in a browser since the CC/SP Timing badges were first added. Do that pass before assuming any of this is bug-free in practice. After that, clean up the leftover `.claude/worktrees/sp-timing-signal` worktree (safe to remove, see Open Questions).
